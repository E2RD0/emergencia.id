...
CREATE INDEX "fkIdx_212" ON "perfil_medico" ("id_estado_isss");

CREATE INDEX "fkIdx_230" ON "perfil_medico" ("id_tipo_sangre");
...
alter table
    perfil_medico
add
    constraint FK_255 foreign key (id_usuario) references usuario on delete cascade;
...
CREATE INDEX "fkIdx_223" ON "usuario_recuperar_clave" ("id_usuario");
...
CREATE INDEX "fkIdx_26" ON "perfil_contacto_emergencia" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_35" ON "perfil_contacto_doctor" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_43" ON "perfil_alergia" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_80" ON "perfil_seguro_medico" ("id_perfil_medico");
...
CREATE TABLE "perfil_procedimiento_medico" (
    ...
    CONSTRAINT "FK_250" FOREIGN KEY ("id_contacto_d") REFERENCES "perfil_contacto_doctor" ("id_contacto_d") ON DELETE
    SET
        NULL,
        CONSTRAINT "FK_91" FOREIGN KEY ("id_perfil_medico") REFERENCES "perfil_medico" ("id_perfil_medico") ON DELETE CASCADE
);

CREATE INDEX "fkIdx_91" ON "perfil_procedimiento_medico" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_74" ON "perfil_medicacion" ("id_perfil_medico");
...
CREATE TABLE "perfil_condicion_medica" (
    ...
    CONSTRAINT "FK_73" FOREIGN KEY ("id_medicacion") REFERENCES "perfil_medicacion" ("id_medicacion") ON DELETE
    SET
        NULL,
        CONSTRAINT "FK_77" FOREIGN KEY ("id_perfil_medico") REFERENCES "perfil_medico" ("id_perfil_medico") ON DELETE CASCADE
);

CREATE INDEX "fkIdx_77" ON "perfil_condicion_medica" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_160" ON "perfil_otra_informacion" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_132" ON "perfil_usuarios_compartir" ("id_perfil_medico");
...
CREATE INDEX "fkIdx_146" ON "perfil_bitacora" ("id_perfil_medico");

CREATE INDEX "fkIdx_152" ON "perfil_bitacora" ("id_accion_bitacora");
...
CREATE INDEX "fkIdx_108" ON "usuario_privilegiado" ("id_tipo_usuario_p");

CREATE INDEX "fkIdx_114" ON "usuario_privilegiado" ("id_organizacion");
...
CREATE
OR REPLACE FUNCTION generate_uid(size INT) RETURNS TEXT AS $ $ DECLARE characters TEXT := 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
...
CREATE
OR REPLACE FUNCTION uid(size INT) RETURNS TEXT AS $ $ DECLARE id TEXT;

count INT;

unico BOOL := FALSE;

BEGIN WHILE NOT unico LOOP
SELECT
    generate_uid INTO id
from
    generate_uid(size);
